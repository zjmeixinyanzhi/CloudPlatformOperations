...
do
  ...
  if [ "${FLAG}" = "0" ];then
    echo "[INFO] controller$i is down!"
    ...
    echo "[INFO] controller$i is up!"
  else
    echo "[ERROR] Get the status of controller$i ariadb is error!"
    exit 127 
  fi
done
...
EOF
  ...
  do
    if [ "controller$i" = $BOOTSTARP_NODE ];then
      ...
      echo "[INFO] controller$i start service:"
